function ACHRSampler_modified(model, warmupPoints, fileName, nFiles, pointsPerFile, stepsPerPoint, initPoint, fileBaseNo, maxTime, printLevel, minFlux, maxFlux, routeFolder)
...
%##########################################################################
%##########################################################################
% NOTE: here a condition for 'printLevel' was added. In this way it's
% possible to provide as input the flux vectors compited via FVA
if nargin<10 || isempty(printLevel)
   printLevel=1; 
end
%##########################################################################
%##########################################################################
% NOTE: This part of the script is disabled because the minFlux and
% maxFlux is obtained via FVA in the 'sampleCbModel_modified' function.
%if nargin<12
%    minFlux = model.lb;
%    maxFlux = model.ub;    
%end
    ...
    while (pointCount <= pointsPerFile)
        ...
        while (stepCount <= stepsPerPoint)
            ...
            if (timeElapsed > maxTime)
                ...
                pathSave = fullfile(routeFolder, file); % This is added to change location for saving results!!
                %save (file,'points');
                save (pathSave,'points');
