function samples = loadSamples_modified(filename, numFiles, pointsPerFile,numSkipped,randPts, routeFolder)
...
for i = 1:numFiles
    if (i > numSkipped)
        ...
        try
            %--------------------------------------------------------------
            % Note: this line is added to indicate location of files to
            % load!
            data = load(fullfile(routeFolder,[filename '_' num2str(i) '.mat']));
            %data = load([filename '_' num2str(i) '.mat']); % original line
            %--------------------------------------------------------------
        catch
